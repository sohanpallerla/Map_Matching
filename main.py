...
def get_closest_points(log, road_rtree, coord_feature_dict):
    ...
    project_roads = []
    # Get all road geometries from the rtree's original list
    road_geoms = road_rtree.geometries
    # Query returns indices of matching geometries
    indices = road_rtree.query(point_buffer)
    for idx in indices:
        road = road_geoms[idx]  # Get the actual geometry using the index
